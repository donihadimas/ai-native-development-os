...
2. Resolve the active task without reading every task body: use an explicit task ID/path first, then IDE or conversation context, then `docs/tasks/index.md` when available, then direct task filenames under `docs/tasks/`, then a narrow heading/status search with user-request terms. Exclude `docs/tasks/done/` unless completed-task history is requested.
...
5. Read summaries, headings, status, acceptance criteria, and relevant sections before full document bodies.
6. Read PRD only when user value or acceptance criteria are unclear.
7. Read design docs when the task changes user-facing flows, screens, states, or accessibility.
8. Read architecture or ADRs only when the task touches design decisions or cross-cutting behavior.
9. Read API docs when frontend/backend integration is involved.
10. Search affected code before proposing new abstractions.
11. Stop reading once the plan can be made safely.
...
- Do not read full PRD, architecture, design, review, or plan bodies when summaries or selected sections are sufficient.
- Do not read every ADR unless the task explicitly requires it.
- Do not use stale docs or stale indexes as stronger evidence than verified behavior.
...
- reading full documents before checking summaries or indexes,
