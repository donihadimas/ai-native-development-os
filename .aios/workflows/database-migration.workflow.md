...
- Full mode: use `.aios/references/database-standards.md` and `.aios/templates/migration-plan.template.md`.
- Lite mode or missing config: use `AGENTS.md`, existing schema notes, and available database standards.
...
- Return to `workflows/new-feature.workflow.md` or `workflows/bugfix.workflow.md` after the migration plan is reviewed.
- Use `database-migration` for migration planning and `.aios/references/database-standards.md` for standards.
- Use `workflows/api-contract.workflow.md` and `api-contract-design` when schema changes affect request, response, webhook, or compatibility behavior.
- Use `workflows/security-review.workflow.md` and `Skill: verify` when persisted data is sensitive or affects auth, permissions, payments, billing, subscriptions, or personally sensitive data.
- Use `Skill: task` only after rollout, rollback, and validation checks are clear; update task status to `Status: Done` in-place.
- Use `Skill: verify` and `workflows/review.workflow.md` to verify pre-migration and post-migration behavior.
...
4. Route through `database-migration`.
5. Create `<docsRoot>/database/migrations/MIGRATION-XXX-title.md` using `.aios/templates/migration-plan.template.md` when available.
...
Use `database-migration` when data change planning is the current step. Read `.aios/references/database-standards.md` and `.aios/templates/migration-plan.template.md` for migration plan creation. Return to the primary workflow after review.

## Lite Mode Flow

Use `AGENTS.md`, existing schema notes, and available database standards. Follow the same migration planning checklist manually.
